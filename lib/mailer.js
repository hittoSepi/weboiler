...
function createMailer({ filePath, encryptionSecret, createTransport = nodemailer.createTransport, createResend = key => new Resend(key) }) {
  ...
  function saveSettings(input) {
    ...
    const next = {
      ...
      fromName: text(input.fromName, 120) || 'Weboiler verkkosivu',
  ...
  function messageText(message, isTest = false) {
    if (isTest) return 'Tämä on Weboilerin hallinnasta lähetetty sähköpostiasetusten testiviesti.';
    return [
  ...
  async function send(message, isTest = false, recipient = '') {
    const settings = readRaw();
    if (settings.provider === 'disabled') return { skipped: true };
    const subject = isTest ? 'Weboiler - testiviesti' : `Uusi yhteydenotto: ${message.name}`;
    ...
    if (settings.provider === 'smtp') {
      ...
      await transporter.sendMail({
        ...
        to: recipient || settings.recipient,
        replyTo,
        subject,
        text: body
      });
    ...
    if (settings.provider === 'resend') {
      const resend = createResend(decrypt(settings.resendApiKey));
  ...
  return {
    publicSettings,
    saveSettings,
    sendContact: (message, recipient = '') => send(message, false, recipient),
    sendTest: () => send({}, true)
  };
}
