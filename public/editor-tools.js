...
function openContextMenu(index,x,y) {
  ...
  contextMenu.innerHTML=`<strong>${esc(editorSections()[index]?.title||'Sivu')}</strong>${index>=0?`<button data-context-edit="${index}">Muokkaa osiota</button><button data-action="duplicate" data-section="${index}">Kopioi osio</button><button data-action="up" data-section="${index}">Siirrä ylös</button><button data-action="down" data-section="${index}">Siirrä alas</button><button class="danger" data-action="delete" data-section="${index}">Poista osio</button>`:''}<strong>Lisää osio tämän jälkeen</strong>${Object.entries(TYPES).map(([type,label])=>`<button data-context-add="${type}" data-after="${index}">${label}</button>`).join('')}`;
