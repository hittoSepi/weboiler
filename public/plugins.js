...
async function loadPlugins(){pluginCatalog=(await api('/api/admin/plugins')).plugins;for(const plugin of pluginCatalog)for(const section of plugin.sections||[])TYPES[section.type]=section.label;}
function pluginSectionFields(section,path){
  ...
  section.pluginData??={};
  return `<p class="wide muted">Tämän osion sisältöä hallitaan Lisäosat-sivulla. Osio näkyy julkisesti vain, jos lisäosa on käytössä julkaistussa versiossa.</p>`+(definition.fields||[]).map(item=>field(`${path}.pluginData.${item.key}`,item.label,section.pluginData[item.key]||'',{textarea:item.type==='textarea',wide:true})).join('');
}
function renderPlugins(){
  site.plugins??={};
  app.innerHTML='<p>Lisäosat ovat projektin mukana asennettua luotettua koodia. Käyttöönotto ja sisältö tallentuvat luonnokseen; julkaise muutokset erikseen. Käytöstä poistaminen säilyttää sisällön.</p>'+pluginCatalog.map(plugin=>{
    const data=site.plugins[plugin.id]||{enabled:false};
    return `<section class="panel"><h2>${esc(plugin.name)}</h2><p>${esc(plugin.description)}</p><label><input type="checkbox" data-plugin-toggle="${esc(plugin.id)}"${data.enabled?' checked':''}> Käytössä</label>${(plugin.collections||[]).map(collection=>`<h3>${esc(collection.label)}</h3>${(data[collection.key]||[]).map((item,index)=>`<article class="subpanel"><div class="section-head"><h3>${esc(item.title||'Uusi kohde')}</h3><button type="button" class="small-button" data-plugin-move="${esc(plugin.id)}" data-collection="${esc(collection.key)}" data-index="${index}" data-direction="-1" aria-label="Siirrä kohde ylös"${index===0?' disabled':''}>↑</button><button type="button" class="small-button" data-plugin-move="${esc(plugin.id)}" data-collection="${esc(collection.key)}" data-index="${index}" data-direction="1" aria-label="Siirrä kohde alas"${index===(data[collection.key]||[]).length-1?' disabled':''}>↓</button><button type="button" class="small-button danger" data-plugin-remove="${esc(plugin.id)}" data-collection="${esc(collection.key)}" data-id="${esc(item.id)}">Poista kohde</button></div><div class="field-grid">${collection.fields.map(def=>pluginField(`plugins.${plugin.id}.${collection.key}.${index}.${def.key}`,def,item[def.key]||'')).join('')}</div>${plugin.routePrefix&&item.slug?`<a target="_blank" href="/admin/esikatselu?plugin=${esc(plugin.id)}&amp;item=${encodeURIComponent(item.slug)}">Esikatsele tallennettua työversiota ↗</a>`:''}</article>`).join('')}<button type="button" class="small-button" data-plugin-add="${esc(plugin.id)}" data-collection="${esc(collection.key)}">Lisää ${esc(collection.label.toLowerCase())}-kohde</button>`).join('')}</section>`;
  }).join('');
}
