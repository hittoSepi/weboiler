...
app.use((req, res, next) => { res.setHeader('X-Content-Type-Options','nosniff'); res.setHeader('X-Frame-Options','SAMEORIGIN'); res.setHeader('Content-Security-Policy',"default-src 'self'; img-src 'self' data: https: http:; style-src 'self' 'unsafe-inline'; script-src 'self'; frame-src 'self'; frame-ancestors 'self'; form-action 'self'"); next(); });
...
function cleanSite(input) {
  ...
  validate.list(input.navigation,12,'Navigaatio');
  validate.list(input.elements,40,'Omat elementit');
  const allowed = new Set(['hero','text','features','gallery','carousel','cta','contact']);
  return {
    meta: { siteName:text(input.meta?.siteName,80), title:text(input.meta?.title,140), description:text(input.meta?.description,300), siteUrl:text(input.meta?.siteUrl,500), logo:validate.url(input.meta?.logo,true), logoAlt:text(input.meta?.logoAlt,120) },
...
app.get('/admin/esikatselu',(req,res)=>session(req)?res.set('X-Robots-Tag','noindex, nofollow').type('html').send(renderSite(readEditorSite(),{preview:true})):res.redirect('/admin/login'));
app.get(['/admin','/admin/yhteydenotot','/admin/editori','/admin/elementit','/admin/media','/admin/asetukset'],(req,res)=>session(req)?res.sendFile(path.join(PUBLIC,'admin.html')):res.redirect('/admin/login'));
